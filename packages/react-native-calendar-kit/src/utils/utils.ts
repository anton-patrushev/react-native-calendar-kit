 ...
 * this produces a fixed-size window (e.g. 5 pages) that can be recentered
 * as the user scrolls, giving infinite-scroll UX with minimal overhead.
 *
 * Delegates to prepareCalendarRange with computed min/max boundaries.
 */
export const prepareCalendarWindow = (
  ...
  if (!isSingleDay) {
    // Week mode: compute exact min/max and delegate to prepareCalendarRange.
    // centerWeekStart is already aligned to firstDay, so prepareCalendarRange
    // won't adjust the boundaries — count will be exactly windowSize weeks.
    const centerWeekStart = startOfWeek(center, firstDay);
    const minDate = centerWeekStart.minus({ weeks: halfWindow }).toISODate();
    const maxDate = centerWeekStart
      .plus({ weeks: windowSize - halfWindow })
      .minus({ days: 1 })
      .toISODate();
  ...
  // Day mode: walk outward from center to find exact visible-day boundaries,
  // accounting for hidden weekdays. This guarantees exactly windowSize pages.
  const hiddenSet = new Set(hideWeekDays ?? []);

  // Walk backward to find minDate (halfWindow visible days before center)
  let minDt = center;
  let found = 0;
  while (found < halfWindow) {
    minDt = minDt.minus({ days: 1 });
    if (!hiddenSet.has(minDt.weekday as WeekdayNumbers)) {
      found++;
    }
  }

  // Walk forward to find maxDate (remaining visible days after center)
  let maxDt = center;
  found = 0;
  const afterCount = windowSize - halfWindow - 1;
  while (found < afterCount) {
    maxDt = maxDt.plus({ days: 1 });
    if (!hiddenSet.has(maxDt.weekday as WeekdayNumbers)) {
      found++;
    }
  }

  return prepareCalendarRange({
    minDate: minDt.toISODate(),
    maxDate: maxDt.toISODate(),
    firstDay,
    isSingleDay: true,
    hideWeekDays,
    timeZone,
